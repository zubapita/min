...
abstract class AppCtl
{
     ...
     */
    private static $table_instances = array();
     ...
     * モデルのインスタンスを保存する配列
     */
    private static $model_instances = array();
     ...
     */
    public function getGETNumValue($name, $defaultValue=0)
    {
     ...
     * @return mix 取得したGET変数の値。もしくは初期設定値。
     */
    public function getGETStrValue($name, $defaultValue='')
    {
        if (isset($_GET[$name])) {
     ...
     */
    public function getPOSTNumValue($name, $defaultValue=0)
    {
     ...
     * @return mix 取得したPOST変数の値。もしくは初期設定値。
     */
    public function getPOSTStrValue($name, $defaultValue='')
    {
        if (isset($_POST[$name])) {
